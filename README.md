...
- 웹 폼으로 추가·수정·삭제, 드래그 및 위/아래 버튼으로 재배치, 상위 과제 변경
...
- 진행 과제 / 보관된 과제 / 전체 과제를 분리하여 조회
- 간트 과제명의 손잡이를 위·아래로 끌어 형제 순서 변경
- 간트 막대를 좌우로 끌어 시작·목표·실제 종료일을 같은 일수만큼 이동
- 간트 막대에 마우스를 올리거나 키보드 포커스하면 최근 기록 요약 표시

완료 상태에서도 실제 종료일은 선택 사항이며 비워 둘 수 있습니다.
과제 삭제는 해당 과제와 모든 하위 작업을 index에서 제거합니다. 기존 MD·JSON·이미지는 Git 복구를 위해 물리 삭제하지 않습니다.
간트 일정 이동은 마일스톤과 릴리즈 날짜를 자동 변경하지 않습니다.

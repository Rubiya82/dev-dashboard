...
- 간트 일정 이동은 기본 잠금: 잠금 해제 → 드래그 → 손을 뗀 뒤 변경 날짜 확인 → 적용/취소 후 자동 재잠금
- 실행 취소/다시 실행: 최근 30단계, Ctrl+Z / Ctrl+Shift+Z 또는 Ctrl+Y (폼 입력 중에는 브라우저 기본 동작)
- 선택 삭제: 체크박스와 표시된 과제 전체 선택, 하위 작업 포함 확인, 일괄 삭제를 한 번에 실행 취소
- 목록·간트에서 하위 과제 접기/펼치기 및 모두 접기/펼치기
- 다크 / 라이트 / 시스템 테마 선택 (이 브라우저에 설정 저장)

실행 취소 이력은 현재 탭의 메모리에만 유지하며 새로고침/작업 폴더 재연결 시 초기화됩니다.
파일 저장 후 실행 취소해도 디스크나 Git이 자동으로 되돌아가지는 않습니다. 변경을 다시 저장·커밋·푸시해야 합니다.
일정 이동 잠금 상태에서는 터치 스크롤이 가능하며, 잠금 해제 상태에서도 10픽셀 미만의 움직임은 무시합니다.
